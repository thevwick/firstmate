...
config/mobile-lab.json  optional per-repo mobile-lab map read by bin/fm-mobile-lab.sh (clone, run_command, metro_port_base, pool_size, sim_device/sim_runtime); LOCAL, gitignored; absent means the mobile lab is inert; template at docs/examples/mobile-lab.json (docs/mobile-lab.md)
...
state/               volatile runtime signals; gitignored
  ...
  lab-build-<slot>.json  bin/fm-mobile-lab.sh build status for one slot, rewritten atomically on every phase change and at least every FM_MOBILE_LAB_HEARTBEAT_SECS; read by fm-console's activity panel (docs/mobile-lab.md)
...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The six MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, the fork-vs-upstream drift check, the local secondmate fast-forward sweep, the secondmate liveness sweep, and X-mode artifact writes - run only when this session actually holds the lock from step 1.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `NUDGE_SECONDMATES:`, `UPSTREAM_SYNC:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
