...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  ...
  # The poll timed out without ever seeing a genuine isolated worktree. When it
  # did observe a cwd, hand that one to validate_spawn_worktree so the refusal
  # names the offending path and the primary checkout - the actionable
  # diagnostic - instead of only the generic timeout. A poll that saw no cwd at
  # all has nothing to name, so it keeps the timeout message.
  if [ -z "$WT" ] && [ -n "$last_seen" ]; then
    WT="$last_seen"
